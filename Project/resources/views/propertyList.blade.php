    ...
    <div class="container-fluid">
        ...
        <div class="right-column">
            ...
            <div class="container-fluid property-listings">
                ...
                $endIndex = min($startIndex + $perPage, count($properties));
                ?>
                <div class="row">
                    @for ($i = $startIndex; $i < $endIndex; $i++)
                        <div class="col-md-6">
                            <div class="property-item rounded fadeInUp props" data-wow-delay="0.1s">
                                <div class="d-flex justify-content-center" style="height: 300px;">
                                    <div class="position-relative" class="property-link">
                                        <a href="#" class="property-link" data-toggle="modal" data-target="#propertyModal" data-property-id="{{ $properties[$i]['id'] }}">
                                            <img class="p-10 img-fluid" src="{{ $properties[$i]['imgSrc'] }}" alt="Property Image">
                                        </a>
                                        <div class="bg-primary rounded text-white position-absolute start-0 top-0 m-4 py-1 px-3">{{ $properties[$i]['statusText'] }}</div>
                                    </div>
                                </div>
                                <div class="p-4 pb-0 text-center">
                                    <h5 class="text-primary mb-3">{{ $properties[$i]['price'] }}</h5>
                                    <h5 class="mb-3">{{ $properties[$i]['address'] }}</h5>
                                    <p><i class="fa fa-map-marker-alt text-primary me-2"></i>{{ $properties[$i]['addressStreet'] }}, {{ $properties[$i]['addressCity'] }}, {{ $properties[$i]['addressState'] }}</p>
                                </div>
                                ...
                                <div class="image-list" data-images="{{ json_encode($properties[$i]['imageUrls']) }}"></div>
                            </div>
                        </div>
                    @endfor
                    <!-- Pagination -->
            <nav aria-label="Page navigation" class="mt-5">
                <ul class="pagination justify-content-center">
                    @php
                    $numPages = ceil(count($properties) / $perPage);
                    @endphp
